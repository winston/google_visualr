...
The most recent update, to GoogleVisualr, 0.7, is not a drop-in replacment for previous versions.  {GoogleVisualr reference site}[http://googlevisualr.heroku.com/]. Specifically, a number of configuration options have been changed to reflect the current API. When looking at the configuration options on the Google documentation, not that all periods have been replaced with underscores. For example, the 'hAxis.title' option can be set by:
